...
@Component({
  selector: 'app-women-item-list',
  standalone: true,
  imports: [CommonModule , HttpClientModule , FormsModule],
  templateUrl: './women-item-list.component.html',
  styleUrl: './women-item-list.component.css'
})
export class WomenItemListComponent implements OnInit {
  items: any[] = [];
  filterText : string = '';
  filterdata : any[] = [];
  filterWom : any[] = [];
   ...
   this.http.get<any>("http://localhost:3000/api/items").subscribe(
    (response) => {
      ...
      if (Array.isArray(response)) {
        this.filterdata = response.filter((item: any) => item.category !== "Woman" && item.discount !== 75);
        this.filterWom = response.filter((item: any) => item.category === "Woman" && item.discount !== 75);
  ...
  filterItems(): void {
    ...
    this.filterdata = this.items.filter(item => item.name.toLowerCase().includes(searchText));
    this.filterWom = this.filterdata.filter(item => item.category === "Woman");
  ...
  prevSlide() {
    this.currentIndex =
      (this.currentIndex - 1 + this.sliderItems.length) % this.sliderItems.length;
  }
